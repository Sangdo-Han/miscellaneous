...
def abort(args):
    ...
    is_killed = getpass.getpass(prompt="Are you sure to kill? y/[N]")
    if is_killed.lower() in ["y", "yes"]:
        for pid in pids:
            proc = subprocess.Popen(
                ['sudo', 'kill', '-9', f'{pid}'],
                stdin=subprocess.PIPE,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--db_path", type=str, default="./vector_store"
    )
    parser.add_argument(
        "--port", "-p", type=str, default="8000"
    )
    parser.add_argument(
        "--exec", "-e", choices=["run", "stop"]
    )
